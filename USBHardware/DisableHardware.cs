...
namespace USBHardware
{
    public static class DisableHardware
    {
        ...
        private const uint PROP_HARDWAREID = 1;
        ...
        public static void ResetDevice(Func<string, bool> filter)
        {
            DisableDevice(filter);
            EnableDevice(filter);
        }

        public static void EnableDevice(Func<string, bool> filter)
        {
            DisableDevice(filter, false);
        }

        public static void DisableDevice(Func<string, bool> filter)
        {
            DisableDevice(filter, true);
        }

        private static void DisableDevice(Func<string, bool> filter, bool disable = true)
        {
            ...
            try
            {
                ...
                for (uint i = 0; ; i++)
                {
                    ...
                    string devicepath = GetStringPropertyForDevice(info, devdata, PROP_HARDWAREID);
